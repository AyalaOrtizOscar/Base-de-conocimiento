...
# -*- coding: utf-8 -*-
"""
feature_analysis_drilling_v3.py

Versión extendida del extractor de features:
 - añade múltiples características adicionales (flatness, bandwidth, contrast, chroma, tonnetz,
   onset_rate, tempo, harmonic/pperc ratio, spectral entropy, crest factor, ...)
 - guarda arrays/plots adicionales
 - calcula matrices de correlación (Pearson y Spearman) globales y por grupos, y guarda heatmaps
 - si export_root contiene 'v2' lo transforma a 'v3' automáticamente para mantener tu petición
 - mantiene la estructura general previa (features, plots, arrays, wavelets, summary)
"""
import argparse
...
# -------------------------
def compute_additional_features(y, sr, n_fft=2048, hop_length=512, n_mels=64):
    feats = {}
    # Spectral flatness (media + std)
    sfm = librosa.feature.spectral_flatness(y=y, n_fft=n_fft, hop_length=hop_length)
    feats["spectral_flatness_mean"] = float(np.mean(sfm)) if sfm.size>0 else np.nan
    feats["spectral_flatness_std"] = float(np.std(sfm)) if sfm.size>0 else np.nan

    # Spectral bandwidth
    spec_bw = librosa.feature.spectral_bandwidth(y=y, sr=sr, n_fft=n_fft, hop_length=hop_length)
    feats["spectral_bandwidth_mean"] = float(np.mean(spec_bw)) if spec_bw.size>0 else np.nan
    feats["spectral_bandwidth_std"] = float(np.std(spec_bw)) if spec_bw.size>0 else np.nan

    # Spectral contrast
    contrast = librosa.feature.spectral_contrast(y=y, sr=sr, n_fft=n_fft, hop_length=hop_length, n_bands=6)
    feats["spectral_contrast_mean"] = float(np.mean(contrast)) if contrast.size>0 else np.nan

    # Chroma
    chroma = librosa.feature.chroma_stft(y=y, sr=sr, n_fft=n_fft, hop_length=hop_length, n_chroma=12)
    feats["chroma_mean_first"] = float(np.mean(chroma[0])) if chroma.size>0 else np.nan
    feats["chroma_mean"] = float(np.mean(chroma)) if chroma.size>0 else np.nan
    feats["chroma_std"] = float(np.std(chroma)) if chroma.size>0 else np.nan

    # Tonnetz (requires harmonic)
    y_harmonic, y_percussive = librosa.effects.hpss(y)
    try:
        tonnetz = librosa.feature.tonnetz(y=y_harmonic, sr=sr)
        feats["tonnetz_0_mean"] = float(np.mean(tonnetz[0])) if tonnetz.size>0 else np.nan
    except Exception:
        feats["tonnetz_0_mean"] = np.nan

    # Harmonic / percussive energy ratio
    harm_energy = np.sum(np.abs(y_harmonic)**2)
    perc_energy = np.sum(np.abs(y_percussive)**2)
    feats["harmonic_percussive_ratio"] = float(harm_energy / (perc_energy + 1e-12))

    # Tempo and onset rate
    tempo, beats = librosa.beat.beat_track(y=y, sr=sr, hop_length=hop_length)
    onsets = librosa.onset.onset_detect(y=y, sr=sr, hop_length=hop_length)
    feats["tempo"] = float(tempo)
    duration_s = len(y) / sr if sr>0 else np.nan
    feats["onset_rate"] = float(len(onsets) / (duration_s + 1e-12))

    # Spectral entropy (compute from mel power)
    S = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=n_mels, n_fft=n_fft, hop_length=hop_length, power=1.0)
    # normalize per-frame
    S_norm = S / (np.sum(S, axis=0, keepdims=True) + 1e-12)
    entropy_per_frame = -np.sum(S_norm * np.log2(S_norm + 1e-12), axis=0)
    feats["spectral_entropy_mean"] = float(np.mean(entropy_per_frame)) if entropy_per_frame.size>0 else np.nan

    # Crest factor (peak / rms)
    rms = np.sqrt(np.mean(y**2) + 1e-12)
    peak = np.max(np.abs(y)) if y.size>0 else 0.0
    feats["crest_factor"] = float((peak + 1e-12) / (rms + 1e-12))

    # Save some arrays to return (chroma, contrast) for plotting/saving later
    return feats, {"chroma": chroma, "contrast": contrast, "y_harmonic": y_harmonic, "y_percussive": y_percussive}
...
def save_heatmap_and_csv(mat: pd.DataFrame, csv_path: Path, png_path: Path, title=None):
    ensure_dir(csv_path.parent)
    ensure_dir(png_path.parent)
    mat.to_csv(str(csv_path))
    try:
        ...
        print("Warning saving heatmap", png_path, e)
...
def process_all(input_root: Path, export_root: Path, sr=44100, n_fft=2048, hop_length=512, n_mels=64, n_mfcc=13, max_files=None):
    ...
    # Si la ruta contiene 'v2' la ajustamos a 'v3' automáticamente (petición del usuario)
    exp_str = str(export_root)
    if "v2" in exp_str.split(os.sep)[-1]:
        export_root = Path(os.sep.join(exp_str.split(os.sep)[:-1] + [exp_str.split(os.sep)[-1].replace("v2", "v3")]))
        print(f"Nota: export_root con 'v2' detectado — usando {export_root}")
    ...
    for p in tqdm(wavs, desc="Archivos"):
        try:
            ...
            # nuevas features
            add_feats, extra_arrays = compute_additional_features(y, sr, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels)
            ...
            # Save chroma & contrast arrays + pngs (nuevas)
            try:
                chroma = extra_arrays.get("chroma")
                contrast = extra_arrays.get("contrast")
                chroma_npy = array_dir / (p.stem + "_chroma.npy")
                chroma_png = plot_dir / (p.stem + "_chroma.png")
                if chroma is not None and chroma.size>0:
                    np.save(str(chroma_npy), chroma)
                    fig, ax = plt.subplots(figsize=(8,3))
                    img = librosa.display.specshow(chroma, sr=sr, hop_length=hop_length, x_axis='time', y_axis='chroma', ax=ax)
                    fig.colorbar(img, ax=ax)
                    ax.set_title(f"Chroma - {p.name}")
                    fig.tight_layout()
                    fig.savefig(str(chroma_png), dpi=150)
                    plt.close(fig)

                contrast_npy = array_dir / (p.stem + "_contrast.npy")
                contrast_png = plot_dir / (p.stem + "_contrast.png")
                if contrast is not None and contrast.size>0:
                    np.save(str(contrast_npy), contrast)
                    fig, ax = plt.subplots(figsize=(8,3))
                    img = librosa.display.specshow(contrast, sr=sr, hop_length=hop_length, x_axis='time', y_axis='linear', ax=ax)
                    fig.colorbar(img, ax=ax)
                    ax.set_title(f"Spectral Contrast - {p.name}")
                    fig.tight_layout()
                    fig.savefig(str(contrast_png), dpi=150)
                    plt.close(fig)
            except Exception as e:
                print("Warning saving chroma/contrast", e)

            # Registrar features en el registro
            rec = {
                "filepath": str(p),
                "filename": p.name,
            ...
            rec.update({
                ...
                # wavelet summary
                "wavelet_total_energy": float(wv["wavelet_total_energy"])
            })
            # añadir nuevas
            rec.update(add_feats)
    ...
    # Resúmenes estadísticos (global / por grupos)
    feature_cols = [c for c in df.columns if c not in ["filepath", "filename", "condition", "diameter", "experiment", "mic", "sr"]]
    ...
    # -------------------------
    # Correlaciones recomendadas y matrices
    # - guardamos Pearson y Spearman global y por grupos
    # -------------------------
    numeric_df = df.select_dtypes(include=[np.number]).dropna(axis=1, how='all')  # quitar cols no numéricas
    if numeric_df.shape[1] >= 2:
        # Pearson global
        pearson = numeric_df.corr(method='pearson')
        save_heatmap_and_csv(pearson, export_root / "summary" / "correlation_pearson_global.csv", export_root / "summary" / "correlation_pearson_global.png", title="Pearson correlation (global)")

        # Spearman global
        spearman = numeric_df.corr(method='spearman')
        save_heatmap_and_csv(spearman, export_root / "summary" / "correlation_spearman_global.csv", export_root / "summary" / "correlation_spearman_global.png", title="Spearman correlation (global)")

        # Correlaciones por grupos (condition/diameter/mic)
        for group in ["condition", "diameter", "mic"]:
            if group in df.columns:
                grp = df.groupby(group)
                for gname, gdf in grp:
                    gnum = gdf.select_dtypes(include=[np.number]).dropna(axis=1, how='all')
                    if gnum.shape[1] < 2:
                        continue
                    try:
                        pmat = gnum.corr(method='pearson')
                        fname_csv = export_root / "summary" / f"correlation_pearson_{group}_{str(gname)}.csv"
                        fname_png = export_root / "summary" / f"correlation_pearson_{group}_{str(gname)}.png"
                        save_heatmap_and_csv(pmat, fname_csv, fname_png, title=f"Pearson correlation ({group}={gname})")

                        smat = gnum.corr(method='spearman')
                        fname_csv = export_root / "summary" / f"correlation_spearman_{group}_{str(gname)}.csv"
                        fname_png = export_root / "summary" / f"correlation_spearman_{group}_{str(gname)}.png"
                        save_heatmap_and_csv(smat, fname_csv, fname_png, title=f"Spearman correlation ({group}={gname})")
                    except Exception as e:
                        print("Warning correlation for group", group, gname, e)
    else:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Extracción masiva de features (v3, ext.)")
    p.add_argument("--input_root", required=True, help="Carpeta raíz con 'Con falla','Sin falla','ruidos'")
    p.add_argument("--export_root", required=True, help="Carpeta de export (ej: D:/v3). Si pasas .../v2 se convertirá a .../v3 automáticamente.")
